 ...
 */
export function BinarySearch_1(nums: number[], target: number) {
    let l = 0
    let r = nums.length
 ...
 * 如果返回值等于0，说明不存在这个值
 */
export function BinarySearch_2(nums: number[], target: number) {
    let l = 0
    let r = nums.length
 ...
 * right不是最后一个节点索引，是
 */
export function lowerBound(check: (mid: number) => boolean, left: number, right: number) {
    while (left < right) {
 ...
 */
export function upperBound(check: (mid: number) => boolean, left: number, right: number) {
    while (left < right) {
...
if (process.argv[2] == '--test') {
    ...
    let target6 = 17
    let test: [number[], number][] = [[num1, target1], [num2, target2], [nums3, target3], [nums4, target4], [nums5, target5], [nums6, target6]]
    let res = []
