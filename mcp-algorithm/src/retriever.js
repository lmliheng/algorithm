 ...
 */
export async function retrieveAlgorithms(config, fileList, query) {
...
async function roughFilter(config, fileList, query) {
...
${fileList.join('\n')}`;
    ...
    return extractJsonArray(content).filter((p) => typeof p === 'string' && allowed.has(p));
}
/** 阶段二：读取候选文件代码，精挑最终结果 */
async function refine(config, candidates, query, maxResults) {
...
async function readCodeBlocks(algorithmDir, candidates) {
    const valid = await Promise.all(candidates.map(async (rel) => {
        let lines;
        try {
    ...
    for (const item of valid) {
        ...
        if (total + lineCount > TOTAL_LINE_LIMIT)
            break;
        total += lineCount;
...
function isValidHit(hit) {
    if (typeof hit !== 'object' || hit === null)
        return false;
    const h = hit;
    return (typeof h.relPath === 'string' &&
        h.relPath.length > 0 &&
